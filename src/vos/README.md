...
设计一个 VOS KV store 需要一个树状数据结构，可以动态增长并保持自我平衡。 需要对树进行平衡，以确保时间复杂度不会随着树大小的增加而增加。 考虑的树数据结构有红黑树和B+树，前者是二叉搜索树，后者是n叉搜索树。
...
尽管与 AVL 树相比，红黑树提供的平衡不那么严格，但它们通过更便宜的重新平衡成本来弥补。 红黑树在Linux内核、java-util库、C++标准模板库等例子中应用比较广泛。 B+ 树与 B 树的不同之处在于它们没有与其内部节点关联的数据。 这可以有助于在内存页面上放置更多键。 此外，B+树的叶节点是相连的； 这意味着进行全扫描只需要一次线性遍历所有叶节点，与 B 树相比，这可以最大限度地减少缓存未命中以访问数据。
...
为了支持上一节（键值存储支持的操作）中提到的更新和打孔，为每个更新或打孔请求设置一个纪元有效性范围以及关联的键，这标志着键从当前纪元开始有效 直到最高可能的纪元。 在未来时期或过去时期对相同密钥的更新修改先前更新的结束时期有效性或相应地打孔。 这样，对于任何给定的键-时期对查找，只有一个键具有有效范围，同时记录键的整个更新历史。 这有助于最近的纪元搜索。 punch 和 update 都有相似的键，除了一个简单的标志来标识查询时期的操作。 查找必须能够在给定的纪元中搜索给定的键并返回关联的值。 除了 epoch-validity 范围之外，DAOS 生成的容器句柄 cookie 也与树的键一起存储。 需要此 cookie 来识别在同一时期发生覆盖时的行为。
...
下表列出了一个用于创建 KV 存储的简单示例输入。 基于 B+ 树的索引和基于红黑树的索引分别显示在下表和下图中。 出于解释的目的，示例中使用了代表性的键和值。
...
红黑树和任何传统的二叉树一样，将小于根的键组织到左子树，将大于根的键组织到右子树。 值指针与每个节点中的键一起存储。 另一方面，基于 B+ 树的索引在叶子上按升序存储键，这是存储值的地方。 根节点和内部节点（相应地以蓝色和栗色进行颜色编码）有助于找到适当的叶节点。 每个B+树节点都有多个槽位，槽位个数由顺序决定。 节点最多可以有 order-1 槽。 在红黑树的情况下，容器句柄 cookie 必须与每个键一起存储，但在 B+ 树的情况下，仅在叶节点中具有 cookie 就足够了，因为遍历时不使用 cookie。
...
在下表中，n 是树中条目的数量，m 是键的数量，k 是键的数量，两个唯一键之间的 epoch 条目。
...
尽管这两种解决方案都是可行的实现，但确定理想的数据结构将取决于这些数据结构在持久内存硬件上的性能。
...
VOS 还支持对这些结构的并发访问，这要求所选择的数据结构在并发更新时提供良好的可扩展性。 与 B+ Tree 相比，红黑树的再平衡会导致更多侵入性的树结构变化； 因此，B+ 树可以提供更好的并发访问性能。 此外，由于 B+ 树节点根据每个节点的大小包含许多槽，因此缓存中的预取可能会更容易。 此外，上表中的顺序计算复杂度表明，与红黑树相比，具有合理顺序的基于 B+ 树的 KV 存储可以执行得更好。
...
VOS 支持枚举在给定时期内有效的密钥。 VOS 提供了一种基于迭代器的方法来从 KV 对象中提取所有键和值。 首先，KV 索引按键排序，然后按纪元排序。 由于每个键都有很长的更新历史，因此树的大小可能会很大。 使用树后继方法的枚举可能导致红黑树的 O(m* log (n) + log (n)) 渐近复杂度，其中 m 是在请求的时期内有效的键数。 它需要 O(log2 (n)) 来定位树中的第一个元素，O(log2 (n)) 来定位后继元素。 因为需要检索“m”个键，所以 O( m * log2 (n)) 将是此枚举的复杂度。
...
B+树的情况下，叶子节点是升序排列的，枚举就是直接解析叶子节点。 复杂度为 O(m * k + logbn)，其中 m 是一个 epoch 中有效的键数，k 是 B+ 树叶节点中两个不同键之间的条目数，b 是 B+ 的顺序 树。 在两个不同的密钥之间具有“k”个纪元条目会导致复杂度为 O(m * k)。 需要额外的 O(logbn) 来定位树中第一个最左边的键。 如上图所示的通用迭代器接口也将用于 KV 枚举。
...
VOS 除了枚举在一个 epoch 内有效的对象的 key，VOS 还支持枚举在两个 epoch 之间修改的对象的 key。 epoch 索引表提供在每个 epoch 更新的键。 在聚合与每个时期关联的密钥列表时，（通过保留密钥的最新更新并丢弃旧版本）VOS 可以生成具有最新时期的密钥列表。 通过在其关联的索引数据结构中查找列表中的每个键，VOS 可以使用基于迭代器的方法提取值。

<a id="73"></a>

## Key Array Stores 键-数组存储
...
VOS 支持的第二种对象是键数组对象。 数组对象，类似于 KV 存储，允许多个版本，并且必须能够同时写入、读取和打孔字节范围的任何部分。 下图显示了键数组对象中范围和纪元排列的简单示例。 在此示例中，不同的线表示存储在相应扩展区中的实际数据，颜色编码指向写入该扩展区范围的不同线程。
...
R-Trees 提供了一种合理的方式来表示范围和时期的有效性范围，从而限制处理读取请求所需的搜索空间。 VOS 提供了一个专门的 R-Tree，称为 Extent-Validity tree (EV-Tree) 来存储和查询版本化数组索引。 在传统的 R-Tree 实现中，矩形是有界的且不可变的。 在 VOS 中，“矩形”由一个轴上的范围和另一个轴上的纪元有效性范围组成。 然而，在插入时 epoch 的有效性范围是未知的，所以所有的矩形都是在假设上限为无穷大的情况下插入的。 最初，DAOS 设计要求在插入时拆分此类树内矩形以限制有效范围，但一些因素导致决定保留原始有效范围。 首先，更新持久内存的成本比查找高一个数量级。 其次，可以通过聚合删除快照之间的覆盖，从而保持相当小的重叠写入历史记录。 因此，EV-Tree 在提取时实现了两部分算法。

1. Find all overlapping extents.  This will include all writes that happened before the requested epoch, even if they are covered by a subsequent write. 查找所有重叠范围。 这将包括在请求的纪元之前发生的所有写入，即使它们被后续写入覆盖。
2. Sort this by extent start and then by epoch 按范围开始排序，然后按纪元排序
3. Walk through the sorted array, splitting extents if necessary and marking them as visible as applicable 遍历排序数组，必要时拆分范围并将它们标记为可见
4. Re-sort the array.  This final sort can optionally keep or discard holes and covered extents, depending on the use case. 重新排序数组。 最后的排序可以根据用例选择保留或丢弃空洞和覆盖范围。
...
TODO：使用上表中的输入为 order-4 EV-Tree 创建一个新图形 Rectangles 表示排列在二维空间中的 extent_range.epoch_validity
...
下图显示了上表示例中EV-Tree的分裂和修剪操作构造的矩形，并在偏移量{0 - 100}处引入了额外的写入以考虑广泛分裂的情况。 上图显示了同一示例的 EV-Tree 构造。
...
EV 树中的插入通过检查重叠找到要插入的适当叶节点。 如果多个边界框重叠，则选择放大最小的边界框。 通过选择面积最小的边界框来解决进一步的关系。 每次插入的最大成本可以是 O(logbn)。
...
搜索 EV-Tree 的工作方式类似于 R-Tree，除了上面描述的错误重叠问题。 必须追求所有重叠的内部节点，直到有匹配的内部节点和叶子。 由于范围可以跨越多个矩形，因此一次搜索可以命中多个矩形。 在理想情况下（整个范围落在一个矩形上），读取成本为 O(logbn)，其中 b 是树的阶数。 排序和拆分阶段增加了 O(n log n) 的额外开销，其中 n 是匹配范围的数量。 在最坏的情况下，这相当于树中的所有范围，但这可以通过聚合来缓解，并且期望与单个键的单个分片关联的树将相对较小。
...
对于从EV-Tree中删除节点，可以使用与搜索相同的方法来定位节点，并且可以删除节点/槽。 删除后，为了合并多个条目少于 order/2 的叶节点，重新插入就完成了。 EV 树重新插入完成（而不是像 B+ 树中那样合并叶节点），因为在删除叶节点/槽时，边界框的大小会发生变化，重要的是要确保矩形被组织成最小边界框而不 不必要的重叠。 在 VOS 中，只有在聚合和丢弃操作时才需要删除。 这些操作将在下一节（基于纪元的操作）中讨论。

<a id="82"></a>
## Conditional Update and MVCC 条件更新和 MVCC
